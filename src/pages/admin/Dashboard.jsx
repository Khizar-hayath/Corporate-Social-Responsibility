...
function Dashboard() {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const [volunteersRes, newsRes, messagesRes, projectsRes] = await Promise.all([
          fetch(`${API_BASE_URL}/admin/volunteers`, { headers }),
          fetch(`${API_BASE_URL}/admin/news`, { headers }),
          fetch(`${API_BASE_URL}/admin/contact`, { headers }),
          fetch(`${API_BASE_URL}/admin/projects`, { headers })
        ]);
        ...
        // Set actual counts from API responses
        setStats({
        ...
        // Update activity feed with latest data if available
        setRecentActivity(mockRecentActivity);
        setUpcomingTasks(mockUpcomingTasks);
        
        // Handle recent unread messages and applications
        const newMessages = Array.isArray(messages) ? messages.filter(msg => msg.status === 'new').length : 0;
        const pendingVolunteers = Array.isArray(volunteers) ? volunteers.filter(vol => vol.status === 'pending').length : 0;
        
        // Update upcoming tasks based on pending items
        if (pendingVolunteers > 0) {
          setUpcomingTasks(prev => [
            { 
              id: 'volunteer-task', 
              title: `Review ${pendingVolunteers} pending volunteer application${pendingVolunteers > 1 ? 's' : ''}`, 
              priority: 'high', 
              dueDate: new Date(Date.now() + 86400000).toISOString() 
            },
            ...prev.filter(task => task.id !== 'volunteer-task')
          ]);
        }
        ...
        if (newMessages > 0) {
          setUpcomingTasks(prev => [
            { 
              id: 'message-task', 
              title: `Respond to ${newMessages} new message${newMessages > 1 ? 's' : ''}`, 
              priority: 'high', 
              dueDate: new Date(Date.now() + 86400000).toISOString() 
            },
            ...prev.filter(task => task.id !== 'message-task')
          ]);
        }
  ...
  const statCards = [
    { title: 'Volunteers', value: stats.volunteers, icon: FiUsers, color: 'bg-blue-500', link: '/admin/volunteers' },
    { title: 'News Articles', value: stats.news, icon: FiFileText, color: 'bg-green-500', link: '/admin/news' },
    { title: 'Messages', value: stats.messages, icon: FiMessageSquare, color: 'bg-yellow-500', link: '/admin/contact' },
    { title: 'Projects', value: stats.projects, icon: FiTarget, color: 'bg-purple-500', link: '/admin/projects' }
  ];
        ...
        >
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Link to="/admin/news" className="flex flex-col items-center justify-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
              ...
              <Link to="/admin/projects" className="flex flex-col items-center justify-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
              ...
              <Link to="/admin/volunteers" className="flex flex-col items-center justify-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
              ...
              <Link to="/admin/contact" className="flex flex-col items-center justify-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                <FiMessageSquare className="w-6 h-6 text-primary-600 dark:text-primary-400 mb-2" />
